...
const OTP_EXPIRE_MS =
  parseInt(process.env.OTP_EXPIRE_MINUTES ?? "10", 10) * 60 * 1000;
...
// ── Secure OTP via crypto.randomInt ───────────────────────────────────────────
...
async function sendEmailOtp(email: string, otp: string): Promise<void> {
  ...
  if (error) {
    ...
    throw new Error(
      `Resend error: ${(error as any)?.message || "unknown"}. If testing locally, set RESEND_TO_EMAIL in .env to your verified Resend email.`
    );
  }
...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    if (!method || !contact?.trim())
      return res
        .status(400)
        .json({ message: "Method and contact are required" });

    if (!["email", "phone"].includes(method))
      return res
        .status(400)
        .json({ message: "Method must be 'email' or 'phone'" });
    ...
    if (method === "email") {
      const found = await User.findOne({ email: contactTrimmed }).select(
        "_id email phone"
      );
      if (found)
        user = {
      ...
      const found = await User.findOne({ phone: contactTrimmed }).select(
        "_id email phone"
      );
      if (found)
        user = {
    ...
    console.log(
      `[OTP SENT] method=${method} contact=${contactTrimmed} otp=${otp}`
    );
...
export const verifyOtp = async (req: Request, res: Response) => {
  try {
    ...
    if (method === "email") {
      const found = await User.findOne({ email: contactTrimmed }).select("_id");
      if (found) userId = String(found._id);
    } else {
      const found = await User.findOne({ phone: contactTrimmed }).select("_id");
      if (found) userId = String(found._id);
    }
    ...
    resetStore.set(resetToken, { userId, expiresAt: tokenExpiry });
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    await User.findByIdAndUpdate(entry.userId, { password: hashedPassword });
