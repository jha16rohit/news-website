...
const BreakingNews: React.FC = () => {
  const {
    articles, deleteArticle, togglePause,
  } = useNews();
  ...
  const breakingArticles = articles.filter(a =>
    a.tagType === "breaking" || a.category === "Breaking News"
  );

  const toDisplayItem = (a: typeof articles[0]) => {
    let displayStatus: "live" | "scheduled" | "expired" | "paused" = "live";
    if (a.statusType === "paused")         displayStatus = "paused";
    else if (a.statusType === "scheduled") displayStatus = "scheduled";
    else if (a.expiryTime && new Date(a.expiryTime) < new Date()) displayStatus = "expired";
    else displayStatus = "live";

    const viewsNum = a.views === "-" ? 0 : parseInt(a.views.replace(/[^0-9]/g, "")) * (a.views.includes("K") ? 1000 : a.views.includes("M") ? 1000000 : 1);

    return {
      id:       a.id,
      headline: a.title,
      author:   `${a.authorFirst} ${a.authorLast}`.trim(),
      timeAgo:  a.published,
      status:   displayStatus,
      category: a.articleCategory || "General",
      views:    viewsNum,
    };
  };

  const displayItems = breakingArticles.map(toDisplayItem);

  const liveCount      = displayItems.filter(n => n.status === "live").length;
  const scheduledCount = displayItems.filter(n => n.status === "scheduled").length;
  const totalViews     = displayItems.reduce((sum, n) => sum + n.views, 0);
  const avgEngagement  = "+24%";

  const filteredNews = displayItems.filter((news) => {
    ...
    const matchesPriority = priorityFilter === "All Priority"; // priority filter kept for UI but no priority column
  ...
  return (
    <div className="bn-container" onClick={() => { setOpenMenuId(null); setIsStatusOpen(false); setIsPriorityOpen(false); }}>
      {/* HEADER */}
            ...
            .map((news) => (
              <div key={news.id} className="bn-live-item">
                ...
                <div className="bn-live-meta">
                  ...
                  <button className="bn-live-link" onClick={() => navigate(`/admin/create?edit=${news.id}&type=breaking`)}>
                    <ExternalLink size={14} />
                  </button>
                </div>
              </div>
            ))}
          {displayItems.filter(n => n.status === "live").length === 0 && (
            <div style={{ padding: "20px", textAlign: "center", color: "#94a3b8", fontSize: "14px" }}>
              No breaking news currently live
            </div>
          )}
        </div>
      </div>
            ...
            {isStatusOpen && (
              <div className="bn-filter-menu">
                {statusOptions.map(opt => (
                  <div key={opt} className={`bn-filter-item ${statusFilter === opt ? "active" : ""}`}
            ...
            {isPriorityOpen && (
              <div className="bn-filter-menu">
                {priorityOptions.map(opt => (
                  <div key={opt} className={`bn-filter-item ${priorityFilter === opt ? "active" : ""}`}
            ...
            {currentNews.map((news) => (
              <tr key={news.id} className="bn-table-row">
                <td className="bn-td-checkbox">
                  <input type="checkbox" checked={selectedItems.includes(news.id)} onChange={() => handleSelectItem(news.id)} />
                </td>
                ...
                <td className="bn-td-actions" onClick={e => e.stopPropagation()}>
                  <div className="bn-action-wrapper">
                    <button className="bn-action-btn" onClick={() => setOpenMenuId(openMenuId === news.id ? null : news.id)}>
      ...
      {totalPages > 1 && (
        <div className="bn-pagination">
          ...
          <div className="bn-pagination-controls">
            <button className="bn-pagination-btn" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}>
              <ChevronLeft size={18} /> Previous
            </button>
            <div className="bn-pagination-pages">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                <button key={page} className={`bn-pagination-page ${page === currentPage ? "active" : ""}`} onClick={() => setCurrentPage(page)}>
                  {page}
                </button>
              ))}
            </div>
            <button className="bn-pagination-btn" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages}>
        ...
        <div className="modal-overlay" onClick={() => setDeleteModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button className="modal-confirm" onClick={() => { deleteArticle(deleteModal); setDeleteModal(null); }}>
                Yes, Delete
              </button>
            </div>
          </div>
