...
const RichEditor: React.FC<RichEditorProps> = ({ editorRef, onInput, wordCount, readTime }) => {
  ...
  return (
    <>
      <div
        ref={editorRef}
        className="cna-editor cna-editor--rich"
        contentEditable
...
interface CustomSelectProps {
  ...
  placeholder?: string; withAvatar?: boolean;
}
const CustomSelect: React.FC<CustomSelectProps> = ({ value, onChange, options, placeholder = "Select...", withAvatar = false }) => {
    ...
    <div className="cna-custom-select" ref={ref}>
      <button type="button" className={`cna-select-trigger${open ? " cna-select-trigger--open" : ""}`} onClick={() => setOpen(p => !p)}>
        <span className="cna-select-trigger-left">
          {withAvatar && label && <span className="cna-author-avatar cna-author-avatar--sm">{label.split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase()}</span>}
        ...
        <ul className="cna-select-dropdown" role="listbox">
          {options.map(opt => (
            <li key={opt.value} role="option" aria-selected={opt.value === value}
              ...
              {withAvatar && <span className="cna-author-avatar cna-author-avatar--sm">{opt.label.split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase()}</span>}
              {opt.label}
            </li>
          ))}
        </ul>
      ...
      <div className="cna-modal cna-modal--schedule" role="dialog" aria-modal="true" aria-labelledby="schedule-modal-title">
        <div className="cna-modal-header">
          <div className="cna-modal-title-wrap">
            ...
            <div>
              <h2 className="cna-modal-title" id="schedule-modal-title">Schedule Article</h2>
                  ...
                  return (
                    <button key={idx} disabled={past}
                      className={[
                        ...
                        (isSun || isSat) && !isSel ? "cna-cal-cell--weekend" : "",
                      ].filter(Boolean).join(" ")}
...
const CreateNewArticle: React.FC = () => {
  ...
  const categoryOptions = categories
    .filter(c => c.enabled)
    .map(c => {
      if (!c.parentId) return { label: c.name, value: String(c.id) };
      const parent = categories.find(p => p.id === c.parentId);
      return { label: parent ? `${parent.name} / ${c.name}` : c.name, value: String(c.id) };
    });

  const getCategoryLabel = () => categoryOptions.find(o => o.value === category)?.label ?? "";
  ...
  const buildBase = () => ({
    title: headline, subtitle: shortTitle || headline.slice(0, 40),
    category: selectedType, articleCategory: getCategoryLabel(),
    authorFirst: "LocalNewz", authorLast: "",
    priority: "Normal" as const, priorityType: "normal",
      ...
      <div className="cna-body">
        <main className="cna-main">
            ...
            <section className="cna-section cna-type-panel cna-type-panel--live">
              ...
              <div className="cna-live-input-row">
                <input className="cna-input" placeholder="Add a new update..." value={liveInput}
                  onChange={e => setLiveInput(e.target.value)} onKeyDown={e => e.key === "Enter" && addLiveUpdate()} />
              ...
              <div className="cna-live-feed">
                {liveUpdates.map(u => (
                  <div key={u.id} className="cna-live-item">
                    ...
                    <button className="cna-live-delete" onClick={() => setLiveUpdates(p => p.filter(x => x.id !== u.id))} aria-label="Delete"><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
            </section>
          ...
          <section className="cna-section cna-editor-section">
            ...
            <RichEditor editorRef={editorRef} onInput={setContent} wordCount={wordCount} readTime={readTime} />
          </section>
          ...
          <div className="cna-card cna-card--status-info">
            ...
            <div className="cna-status-info-list">
              <div className="cna-status-info-item">
                ...
                <div>
                  <p className="cna-status-info-label">Publish Now</p>
              ...
              <div className="cna-status-info-item">
                ...
                <div>
                  <p className="cna-status-info-label">Schedule</p>
