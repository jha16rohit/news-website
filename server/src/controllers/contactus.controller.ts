...
// ── Admin email notification when a user submits the Contact form ─────────────
async function sendContactNotification(msg: {
  ...
  if (!adminEmail) {
    console.warn(
      "[ContactUs] RESEND_TO_EMAIL not set — skipping notification."
    );
    return;
  }
  ...
  const { error } = await getResend().emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:36px;
        ...
        <h2 style="font-size:18px;color:#111;margin:0 0 4px">
          New Contact Message
        </h2>

        <p style="color:#888;font-size:13px;margin:0 0 20px">
          A user submitted a message via the Contact Us page.
        </p>

        <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:20px">
          <tr>
            <td style="padding:7px 0;color:#888;width:110px;vertical-align:top">
              Name
            </td>
          ...
          <tr>
            <td style="padding:7px 0;color:#888;vertical-align:top">
              Email
            </td>
            <td style="padding:7px 0">
              <a href="mailto:${msg.email}" style="color:#e10600">
                ${msg.email}
              </a>
            </td>
          </tr>
          ...
          <tr>
            ...
            <td style="padding:7px 0;color:#111">
              ${msg.phone}
            </td>
          </tr>`
              : ""
        ...
        <div style="background:#f9f9f9;border-left:4px solid #e10600;border-radius:6px;
  ...
  if (error) {
    console.error(
      "[ContactUs] Resend error:",
      (error as any)?.message || error
    );
  } else {
    console.log(
      `[ContactUs] Admin notification sent → ${adminEmail}`
    );
  }
}
...
const DEFAULT_FAQ = [
  {
    ...
    answer:
      "Use the contact form above and select 'News Tip' as the subject, or email us at editorial@localnewz.in.",
    visible: true,
    position: 0,
  },
  {
    question: "How long does it take to get a response?",
    answer: "We aim to respond within 24–48 working hours.",
    visible: true,
    position: 1,
  },
  {
    ...
    answer:
      "Reach out at ads@localnewz.in or fill the contact form selecting 'Advertising' as your subject.",
    visible: true,
    position: 2,
  },
  {
    ...
    answer:
      "Select 'Correction Request' in the form and include the article URL and the specific correction needed.",
    visible: true,
    position: 3,
  },
];
...
export const getContactUsSettings = async (
  ...
  try {
    ...
    const { fAQItem, ...rest } = settings as any;
...
export const updateContactUsSettings = async (
  ...
  try {
    ...
    const updated = await prisma.contactUsSettings.findUnique({
      ...
      include: {
        contactInfo: { orderBy: { position: "asc" } },
        fAQItem: { orderBy: { position: "asc" } },
      },
    });

    const { fAQItem, ...rest } = updated as any;
...
// Public — used by the success screen to poll for admin reply
...
export const createMessage = async (
  ...
  try {
    ...
    sendContactNotification(msg).catch((err) =>
      console.error(
        "[ContactUs] Notification email failed:",
        err.message
      )
    );
...
export const replyToMessage = async (
  ...
  try {
    ...
    try {
      ...
      if (error) {
        console.error(
          "[ContactUs] Reply email failed:",
          error
        );
      } else {
        console.log(
          `[ContactUs] Reply email sent to ${msg.email}`
        );
      }
    } catch (mailErr: any) {
      console.error(
        "[ContactUs] Mail sending error:",
        mailErr.message
      );
    }
